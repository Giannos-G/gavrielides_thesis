...
def GetSerialorParallelProcessing():
    # 0-> Serial
    # 1-> Parallel
    decision = input("Please insert 0 if you want to run your code serial or 1 if you want to run your code in parallel: \n")
...
time_on_nano_table, time_on_jetson_table, decision):     # Sum of energy of each node
    ...
    GetTotalTime(total_time_on_nano, total_time_on_jetson, number_of_nano, number_of_jetson, decision)
...
def GetCostOfCommunications(communications, map_table,functions,nodes,decision): # Every node has the same cost 
...
def GetTotalTime(time_on_nano_table, time_on_jetson_table,nano_nodes, jetson_nodes, decision):
    Total_Time = 0
    ...
    for r in range(int(nano_nodes)):
        if (decision == 0 ):    # Serial 
            ...
            if (time_on_nano_table[0][r] > Total_Time):
                Total_Time = time_on_nano_table[0][r]
    ...
    for r in range(int(jetson_nodes)):
        if (decision == 0 ):    # Serial 
            ...
            if (time_on_jetson_table[0][r] > Total_Time):
                Total_Time = time_on_jetson_table[0][r]
...
def main():
    ...
    serial_or_parallel = GetSerialorParallelProcessing()
